...
- **GW2000:** Home Assistant recorder history is backfilled in 14-day sections, then kept current from the live Home Assistant entities. The normalized station contract includes temperature, feels-like temperature, humidity, dew point, VPD, pressure, live and maximum wind, both wind directions, daily rain, rain intensity, solar radiation, UV, soil moisture/temperature, and sensor battery/capacitor health. A GW2000 leaf-wetness channel is automatically discovered and begins recording when installed; until then the interface says it is ready and does not fabricate a value. Google Drive's Baiamonte Weather CSV can be imported to fill older history and gaps.
...
- **Twilio company voice:** WhatsApp Business Calling, PSTN telephone numbers and SIP/VoIP domains are separate, disabled-by-default inputs to one signed Baiamonte IVR. Configure the TwiML Voice Application, exact public HTTPS `/webhooks/twilio/voice` URL, `PN…` phone-number SIDs and/or authenticated `SD…` SIP-domain SIDs in protected add-on settings. Unknown callers receive bilingual Guest Reception; saved WhatsApp/PSTN contacts retain personalized Manager or Reporter access. SIP callers with an E.164 user part can use the same contact match; other authenticated SIP callers receive Reception. Admin → IVR verifies every configured resource on demand without background polling.
- **Outbound and forwarding:** enable these separately, set a verified E.164 `twilio_outbound_caller_id`, and store one protected E.164 or `sip:` forwarding target. The browser never receives the forwarding destination. An administrator can explicitly confirm an outbound PSTN or SIP call; callers can transfer to reception using 5 (Guest), 9 (Reporter) or 13 (Manager). Every request, channel, route and status is audited, but live Twilio audio is not stored. WhatsApp text, photos and voice notes remain the follow-up channel.
- **Production sender caution:** linking or migrating a production WhatsApp sender can affect its existing Meta messaging registration. Do not change the current Meta sender until a dedicated Twilio sender or an approved migration plan is ready.
...
Port 8102 remains a LAN-only display service. For operation outside the VPN, the managed Home Assistant integration provides a narrow public gateway through the estate's existing Home Assistant Cloud connection. Set `cellar_label_public_origin` to the external Home Assistant origin plus `/api/baiamonte_labels`, for example `https://example.ui.nabu.casa/api/baiamonte_labels`. No router port, separate tunnel or DNS change is required.

The gateway accepts GET requests only for the tokenized tank/kiosk pages, their read-only data, display assets and bootstrap-key-protected enrollment. It cannot relay Vineyard Operations administration, Home Assistant administration or any other app port. The UUID label URL is the bearer credential, is marked no-cache/no-index and must remain private. The larger `ipad` dashboard continues to use normal Home Assistant authentication on the configured external Home Assistant origin; only tank-label display routes are login-free.
...
The Weather page reads the complete normalized GW2000 observation through Home Assistant and distinguishes cumulative daily rain from current rain rate, and live gust from maximum daily gust. Station-native dew point and VPD are preferred; calculated display fallbacks are used only when those channels are unavailable. Weather fields are routed by relevance: disease and treatment models receive moisture, rain intensity, VPD, temperature, wind, solar, soil and phenology context; harvest receives the authoritative daily weather/GDD history and forecast; alerts receive safety-critical extremes; visual and assistant pipelines receive the bounded current context. Its forecast uses the preferred available Home Assistant `weather.*` entity. The large precipitation map reuses the local ADS-B weather-map service so its animated layer and attribution remain intact; if that service or its weather credentials are down, the page reports that state instead of inventing weather.

Set `full_refresh_minutes` from 5–1440 minutes to control the master recovery sweep; 60 minutes is the recommended default. Scheduled sweeps rerun only a subsystem that is missing or at least twice past its normal cadence, so healthy weather, Gmail, finance, disease and alert jobs are not duplicated every hour. The manual **Run complete update now** action still runs every configured subsystem. Projections and TV/dashboard views recalculate from refreshed data, and every sweep or error appears in the Processing Log.
