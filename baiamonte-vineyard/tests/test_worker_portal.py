...
class WorkerPortalTests(unittest.TestCase):
    ...
    def test_admin_and_operations_navigation_preserves_payroll_access(self) -> None:
        ...
        self.assertIn("document.addEventListener('click',handleNavModeClick)", javascript)
        self.assertIn('function activateViewButton(button)', javascript)
        self.assertNotIn("button.onclick=()=>setNavMode(button.dataset.navMode,true)", javascript)
