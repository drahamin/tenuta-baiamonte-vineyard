...
class WhatsappSenderStateTests(unittest.TestCase):
    def test_runtime_settings_are_merged_instead_of_replaced(self) -> None:
        source = (ROOT / "app" / "domains" / "communications_meta_routes.py").read_text(encoding="utf-8")
    ...
    def test_inbound_and_outbound_health_are_sender_scoped(self) -> None:
        main = (ROOT / "app" / "domains" / "communications_meta_routes.py").read_text(encoding="utf-8")
        webhook = (ROOT / "app" / "domains" / "communications_meta_webhook_routes.py").read_text(encoding="utf-8")
        ...
        self.assertIn('"phone_number_id": receiver_phone_number_id or None', webhook)
