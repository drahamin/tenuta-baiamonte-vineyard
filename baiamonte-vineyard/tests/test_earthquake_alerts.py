...
class EarthquakeAlertTests(unittest.TestCase):
    ...
    def test_tv_pressure_shows_lead_risk_action_and_progression(self) -> None:
        ...
        self.assertIn("Agronomist review pending", tv_js)
        self.assertNotIn("Agronomist and Enologist review pending", tv_js)
        self.assertNotIn("Sebastian review pending", tv_js)
        self.assertIn("tv-pressure-grid", tv_css)
        self.assertIn("tv-pressure-roll.rising", tv_css)
        self.assertIn('.screen[data-screen="2"].active{display:flex', tv_css)
        self.assertIn("@media(max-height:800px)", tv_css)
