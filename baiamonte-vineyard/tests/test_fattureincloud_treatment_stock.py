...
def test_recognized_invoice_lines_map_to_canonical_stock_products():
    cases = {
        "30008h SACRON 45 WG KG 1 CIMOXANIL45%": ("SACRON 45 WG", "1", "kg", "candidate"),
        "30038d OSSICLOR 35 WG KG. 10 BIO": ("OSSICLOR 35 WG", "10", "kg", "candidate"),
        "27046 IMPULSIVE LT 1": ("IMPULSIVE PREMIUM", "1", "L", "support"),
        "27040 RESOLVE X 5 LT": ("RESOLVE", "5", "L", "support"),
        "28020 TERRAPLUS SOLUB NPK 8-7-6 15 KG": ("TERRAPLUS SOLUB NPK 8-7-6", "15", "kg", "support"),
        "27043 GEL DI SILICE X 5 KG": ("GEL DI SILICE", "5", "kg", "support"),
    }
    for description, expected in cases.items():
        match = _stock_product_match({"description": description})
        assert match is not None
        assert (match[0], str(match[1]), match[2], match[3]) == expected
...
def test_sync_posts_recognized_lines_idempotently_to_local_stock():
    ...
    assert "TREATMENT_STOCK_AUTHORITY_YEAR = 2026" in source
