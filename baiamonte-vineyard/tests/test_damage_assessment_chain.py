...
def test_damage_chain_is_database_backed_and_editable_in_agronomy():
    ...
    assert "DROP FOREIGN KEY" not in scope_migration
    assert "MODIFY COLUMN block_id" not in scope_migration
    ...
    assert "information_schema.KEY_COLUMN_USAGE" in scope_migration
