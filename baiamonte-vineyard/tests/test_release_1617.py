...
def test_release_version_is_consistent():
    assert 'version: "1.9.85"' in (ROOT / "config.yaml").read_text()
...
def test_nerello_nutriferm_exact_rate_schedules_apa_adjustment():
    ...
    assert "INSERT IGNORE INTO enology_test_requests" in migration
    assert "ON DUPLICATE KEY UPDATE requested_at" not in migration
