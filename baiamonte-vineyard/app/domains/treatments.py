...
def product_guidance(crop_scope: str, prediction: dict[str, Any], *, forecast: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    ...
    purchases = fetch_all("SELECT pe.*,p.name product_name FROM treatment_purchase_evidence pe LEFT JOIN products p ON p.id=pe.product_id WHERE pe.estate_id=%s AND YEAR(pe.invoice_date)=YEAR(CURDATE()) ORDER BY pe.invoice_date,pe.invoice_number,pe.line_number", (estate_id(),))
    ...
    if not target_code:
        return {"status": "waiting_for_target", "target_code": None, "candidates": [], "mixture": None, "purchase_summary": purchase_summary, "non_treatment_purchases": non_treatment, "message": "No current target is supported. Purchased products are inventory evidence, not a reason to spray."}
    ...
    if not candidates:
        return {"status": "no_verified_candidate", "target_code": target_code, "candidates": [], "mixture": None, "blocked_products": blocked_products, "purchase_summary": purchase_summary, "non_treatment_purchases": non_treatment, "message": "No currently authorized crop-and-target product is verified. The engine may recommend a new purchase only after a current Italian label is added and checked."}
    ...
    purchase_state = "purchased_current_year" if candidate.get("product_name") in purchase_by_product else "suggested_purchase"
    ...
    component = {"product_name": candidate.get("product_name"), "active_ingredient": candidate.get("active_ingredient"), "registration_number": candidate.get("registration_number"), "purpose": candidate.get("target_name"), "rate": rate, "rate_unit": candidate.get("dose_unit"), "total": calculation.get("total_kg") if calculation else None, "total_unit": "kg" if calculation else None, "per_100_l": calculation.get("per_100_l_g") if calculation else None, "per_100_l_unit": "g/100 L" if calculation else None, "purchase_state": purchase_state, "phi_days": phi_days, "rei_hours": candidate.get("rei_hours"), "resistance_group": candidate.get("resistance_group"), "mixing_sequence": "Add only according to the current container label while agitation is running; do not add unverified adjuncts.", "label_url": candidate.get("label_url")}
    support_review = [{"product_name": "GEL DI SILICE", "purchase_state": "purchased_current_year", "decision": "blocked_pending_container_check", "reason": "Invoice is kg while the historical recipe is ml/100 L; exact formulation, rate, compatibility and sequence must be reconciled."}, {"product_name": "RESOLVE", "purchase_state": "purchased_current_year", "decision": "blocked_pending_container_check", "reason": "Invoice is liquid 5 L packages while the historical recipe is g/100 L; do not infer a conversion."}, {"product_name": "IMPULSIVE PREMIUM", "purchase_state": "purchased_current_year", "decision": "not_selected", "reason": "Nutritional/biostimulant support is not justified by the current powdery-mildew signal alone."}] if target_code == "powdery_mildew" else []
    return {"status": "calculated_proposal_blocked" if hard_blocks else "ready_for_agronomist_review", "target_code": target_code, "target_name": candidate.get("target_name"), "preferred_candidate": candidate, "candidates": candidates, "blocked_products": blocked_products, "purchase_summary": purchase_summary, "non_treatment_purchases": non_treatment, "application_window": spray_window, "mixture": {"planning_basis": {"area_ha": known_area, "water_l": planning_water_l, "area_note": "All active blocks with known area; confirm exact treated blocks.", "water_note": "Planning assumption; confirm tank volume."}, "components": [component], "support_product_review": support_review, "mixing_order": [component["product_name"]], "hard_blocks": hard_blocks, "earliest_harvest_forecast": earliest_harvest, "phi_passes_current_forecast": phi_ok}, "message": "Calculated decision support, not an application order. Products outside the purchase ledger are allowed and are clearly marked as suggested purchases."}
