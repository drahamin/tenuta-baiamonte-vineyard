...
def build_operational_projections(
    ...
    for forecast_year in sorted({int(row["vintage_year"]) for row in production_forecasts}):
        ...
        forecast_totals.append({"vintage_year": forecast_year, "grape_kg": total_kg, "baseline_grape_kg": baseline_kg, "crates_15kg": round(total_kg / 15), "wine_l": round(total_kg * conversion), "bottles_750ml": int(total_kg * conversion / 0.75), "sources": sorted({str(row.get("source") or "unlabelled") for row in rows})})
    return {
        "year": year,
        ...
        "production_forecast_method": "Database planning records; not a learned forecast model. Provisional, deduplicated scouting damage adjusts the baseline.",
        "grape_allocations": [
            {
