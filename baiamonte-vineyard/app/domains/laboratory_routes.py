...
def create_lab_sample(payload: LabSampleCreate, year: int = Query(default_factory=lambda: date.today().year)) -> dict[str, Any]:
    ...
    possible_duplicates = fetch_all(
        ...
        "AND LOWER(TRIM(sample_name))=LOWER(TRIM(%s)) AND vintage_year=%s "
        "AND (variety_id <=> %s) AND (wine_lot_id <=> %s) AND (LOWER(TRIM(laboratory)) <=> LOWER(TRIM(%s)))",
        (estate_id(), payload.sample_type, payload.lab_date, payload.sample_name, sample_year, payload.variety_id, payload.wine_lot_id, payload.laboratory),
    )
    ...
    return {"id": record_id, "prediction_refresh": "queued" if payload.sample_type == "grape" else "not_applicable", "duplicate": False}
