...
function render(d){
  ...
  $('tvSystemLights').innerHTML=(d.system_status?.services||[]).map(item=>`<div class="tv-system-light ${esc(item.state)}" title="${esc(item.detail)}"><i></i>${esc(item.name)}</div>`).join('');
