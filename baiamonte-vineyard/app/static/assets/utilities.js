...
(function () {
  ...
  function renderWater(data) {
    ...
    $('waterKpis').innerHTML = [metric('Cistern level', numeric(level.level_percent) == null ? '—' : `${fmt(level.level_percent)}%`, level.label || 'Camera evidence', Number(level.level_percent) < 10 ? 'alert-metric' : ''), metric('Reading confidence', confidence == null ? '—' : `${Math.round(confidence * 100)}%`, stamp(level.observed_at)), metric('Water entities', data.health?.connected ?? 0, `${data.health?.unavailable || 0} unavailable`), metric('Learning status', String(learning.model_status || 'commissioning').replaceAll('_', ' '), `${learning.observation_count || 0} observations`)].join('');
