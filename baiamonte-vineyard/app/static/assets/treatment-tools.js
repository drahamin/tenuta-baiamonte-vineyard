(function treatmentDecisionTools(){
  ...
  function renderSimulatorResult(result){
    const node=$('treatmentSimulatorResult'),prediction=result.prediction||{},guidance=result.product_guidance||{},mix=guidance.mixture||{},components=mix.components||[],program=mix.program_components||components,passes=mix.program_passes||[],needed=guidance.needed_list||[],review=result.field_review_guidance||{},readiness=result.inventory_readiness||{},batches=mix.batch_recipe||[],support=mix.support_product_review||[],candidates=guidance.candidates||[],blocked=guidance.blocked_products||[],application=guidance.application_window||{},weather=guidance.weather_watch||{},historical=result.historical_comparison||[];
    if(!node)return;
