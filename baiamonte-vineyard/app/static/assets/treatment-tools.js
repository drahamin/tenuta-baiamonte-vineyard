(function treatmentDecisionTools(){
  ...
  function renderSimulatorResult(result){
    const node=$('treatmentSimulatorResult'),prediction=result.prediction||{},guidance=result.product_guidance||{},mix=guidance.mixture||{},components=mix.components||[],needed=guidance.needed_list||[],review=result.field_review_guidance||{},readiness=result.inventory_readiness||{},batches=mix.batch_recipe||[],support=mix.support_product_review||[],candidates=guidance.candidates||[],blocked=guidance.blocked_products||[],application=guidance.application_window||{};
    if(!node)return;
    ...
    ${mix.mixing_order?.length?`<p><b>Provisional mixing order:</b> ${mix.mixing_order.map(esc).join(' → ')}. Confirm the current labels, water conditions and exact compatibility before mixing.</p>`:''}
