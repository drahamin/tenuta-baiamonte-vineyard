(function treatmentDecisionTools(){
  ...
  window.openProductLabelIntakeReview=function(item,data,id){const suggestions=Array.isArray(data.suggested_database_records)?data.suggested_database_records:[],fields=suggestions[0]?.fields||suggestions[0]?.values||{},uncertainties=Array.isArray(data.uncertainties)?data.uncertainties:[],value=(...keys)=>{for(const key of keys)if(fields[key]!=null)return fields[key];return''},expected=String(value('product_name','name')||'').toLowerCase(),products=state.reference?.products||[];$('intakeDialogTitle').textContent=item.title||item.original_filename||'Product information';$('intakeDetail').innerHTML=`<p>${esc(data.summary||item.ai_summary||'AI product analysis ready for review.')}</p>${uncertainties.length?`<article class="panel muted-panel"><h3>Conflicts or missing evidence</h3><p>${uncertainties.map(item=>esc(factText(item))).join('<br>')}</p></article>`:''}<form id="productEvidenceApprovalForm" class="completion-form"><p class="safety-note">Approval stores source-linked evidence. It does not silently rewrite inventory, labels, treatment recipes, or density.</p><label>Estate product<select name="product_id" required><option value="">Choose matching product</option>${products.map(row=>`<option value="${esc(row.id)}" ${String(row.name||'').toLowerCase()===expected?'selected':''}>${esc(row.name)}</option>`).join('')}</select></label><label>Evidence type<select name="evidence_type"><option value="container_label">Container label</option><option value="manufacturer_label">Manufacturer label</option><option value="sds">Safety sheet / SDS</option><option value="technical_product_page">Technical sheet</option><option value="owner_document">Other owner document</option></select></label><div class="field-row"><label>Formulation<input name="formulation" value="${esc(value('formulation','observed_form'))}" placeholder="liquid, powder, WG…"></label><label>Lot<input name="lot_number" value="${esc(value('lot_number','lot'))}"></label></div><div class="field-row"><label>Minimum rate<input name="rate_min" type="number" min="0" step="0.001" value="${esc(value('rate_min','observed_rate'))}"></label><label>Maximum rate<input name="rate_max" type="number" min="0" step="0.001" value="${esc(value('rate_max'))}"></label><label>Rate unit<input name="rate_unit" value="${esc(value('rate_unit','observed_rate_unit'))}" placeholder="L/ha, ml/100 L…"></label></div><label>Reviewed evidence notes<textarea name="notes" rows="5">${esc([value('directions','notes'),...uncertainties.map(factText)].filter(Boolean).join('\n'))}</textarea></label><div class="form-actions"><a class="button secondary" href="api/v1/intake/${esc(id)}/file" target="_blank" rel="noopener">View source</a><button type="submit">Approve product evidence</button></div></form>`;$('intakeDialog').showModal();const form=$('productEvidenceApprovalForm');form.onsubmit=async event=>{event.preventDefault();const button=event.submitter,payload=Object.fromEntries(new FormData(form).entries());button.disabled=true;try{const result=await api(`api/v1/treatments/product-evidence/intake/${encodeURIComponent(id)}/approve`,{method:'POST',body:JSON.stringify(payload)});$('intakeDialog').close();toast(`${result.product_name} evidence approved`);await loadAll()}catch(error){toast(error.message)}finally{button.disabled=false}}};
