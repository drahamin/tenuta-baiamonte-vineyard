...
(function () {
  ...
  const openVineyardVisualSnapshot = watch => {
    ...
    $('recordDialogList').innerHTML = `<section class="cistern-snapshot-view"><div class="cistern-snapshot-frame"><img src="${visualSnapshotUrl(watch)}" alt="Latest Vineyard North fixed-view observation"></div><div class="cistern-snapshot-facts"><span><small>Finding</small><b>${esc(watch.status_label || 'Learning')}</b></span><span><small>Canopy signal</small><b>${esc(watch.canopy_trend || 'Building baseline')}</b></span><span><small>Mount Etna</small><b>${esc(watch.etna_summary || 'Not yet assessed')}</b></span><span><small>Etna visibility</small><b>${esc(watch.etna_visibility || 'Not assessed')}</b></span><span><small>Scene change</small><b>${watch.frame_change_pct == null ? 'First frame' : esc(fmt(watch.frame_change_pct)) + '%'}</b></span><span><small>View visibility</small><b>${esc(watch.visibility || 'Not assessed')}</b></span></div><p class="safety-note">${esc(watch.summary || 'No interpretation available.')}</p><p class="safety-note">Visual Etna screening is corroborated against current INGV and ash-advisory evidence before an alert. ${esc(watch.evidence_note || 'Confirm visual changes in the field before action.')}</p></section>`;
  ...
  window.renderVineyardVisualWatch = function () {
    ...
    const facts = [['Canopy signal', watch.canopy_trend || 'Building baseline'], ['Scene change', watch.frame_change_pct == null ? 'First frame' : `${fmt(watch.frame_change_pct)}%`], ['Visibility', watch.visibility || 'Not assessed'], ['Etna summit', watch.etna_visibility === 'clear' ? (watch.etna_activity === 'none' ? 'Visible · no unusual activity' : watch.etna_summary) : watch.etna_visibility || 'Not assessed'], ['Activity', watch.operations || 'No reviewed observation'], ['Updated', captured]];
...
function renderAdminAi(data=state.adminAi){
  ...
  const service=data.ai_service||{},learning=data.learning||{},summary=learning.summary||{};
  renderCameraAiPolicy(data.camera_ai||{});
  $('adminAiServiceLights').innerHTML=`<div class="system-light ${service.status==='available'?'green':service.status==='blocked'?'red':'amber'}"><i></i><span><b>OpenAI API</b><small>${esc(service.status==='available'?'Available':service.detail||String(service.status||'unverified').replaceAll('_',' '))}</small></span></div><div class="system-light ${learning.overall_status==='healthy'?'green':learning.overall_status==='attention'?'red':'amber'}"><i></i><span><b>Durable learning</b><small>${esc(String(learning.overall_status||'checking').replaceAll('_',' '))}</small></span></div>`;
  ...
  budgetForm.onsubmit=async event=>{event.preventDefault();const button=event.submitter;button.disabled=true;try{await api('api/v1/admin/ai-cost',{method:'PUT',body:JSON.stringify({monthly_budget_usd:Number(budgetForm.elements.monthly_budget_usd.value),warning_percent:Number(budgetForm.elements.warning_percent.value)})});toast('AI budget saved');await loadAdminAi()}catch(error){toast(error.message)}finally{button.disabled=false}};
  ...
  const featureRows=(cost.by_feature||[]).map(row=>`<div class="list-row"><span class="list-icon">AI</span><div><b>${esc(featureNames[row.feature_code]||String(row.feature_code).replaceAll('_',' '))}</b><small>${Number(row.requests||0)} request${Number(row.requests||0)===1?'':'s'} · ${fmt(row.input_tokens||0)} in / ${fmt(row.output_tokens||0)} out</small></div><time>${usd(row.estimated_cost_usd)}</time></div>`).join(''),modelRows=(cost.models||[]).map(row=>`<div class="list-row ai-model-row"><span class="list-icon">M</span><div><b>${esc(row.model||'Unknown model')}</b><small>${Number(row.requests||0)} requests · $${Number(row.input_usd_per_million||0).toFixed(2)} input / $${Number(row.output_usd_per_million||0).toFixed(2)} output per 1M</small></div></div>`).join('');$('adminAiBreakdown').innerHTML=(featureRows+modelRows)||'<div class="list empty">Usage tracking begins with this version.</div>';
  $('adminLearningOverall').textContent=String(learning.overall_status||'checking').replaceAll('_',' ');
  $('adminLearningSummary').innerHTML=`<article class="metric"><span>Processes</span><strong>${Number(summary.model_count||0)}</strong></article><article class="metric"><span>Validated</span><strong>${Number(summary.validated||0)}</strong></article><article class="metric"><span>Learning</span><strong>${Number(summary.learning||0)+Number(summary.waiting||0)}</strong></article><article class="metric ${Number(summary.attention||0)+Number(summary.unavailable||0)?'alert-metric':''}"><span>Needs attention</span><strong>${Number(summary.attention||0)+Number(summary.unavailable||0)}</strong></article>`;
  const metricValue=metric=>metric?.value===null||metric?.value===undefined?'Not measured':`${fmt(metric.value)}${metric.unit||''}`;
  $('adminLearningModels').innerHTML=(learning.models||[]).map(model=>`<article class="learning-model-card ${esc(model.status||'waiting')}"><header><div><small>${esc(model.domain||'System')}</small><h4>${esc(model.name)}</h4></div><span><i></i>${esc(model.status_label||model.status||'Waiting')}</span></header><div class="learning-primary"><small>${esc(model.primary_metric?.label||'Validation')}</small><strong>${esc(metricValue(model.primary_metric))}</strong><em>${esc(model.primary_metric?.target||'')}</em></div><div class="learning-metrics">${(model.metrics||[]).map(metric=>`<span><small>${esc(metric.label)}</small><b>${esc(metricValue(metric))}</b>${metric.target?`<em>${esc(metric.target)}</em>`:''}</span>`).join('')}</div><dl><div><dt>Model</dt><dd>${esc(model.model_version||'—')}</dd></div><div><dt>Data through</dt><dd>${esc(model.data_through||'—')}</dd></div><div><dt>Last rebuilt</dt><dd>${esc(model.trained_at?timeLabel(model.trained_at):'—')}</dd></div></dl><p>${esc(model.validation_method||'No validation method recorded.')}</p>${(model.issues||[]).length?`<ul>${model.issues.map(issue=>`<li>${esc(issue)}</li>`).join('')}</ul>`:'<div class="admin-ok">Evidence gate currently satisfied.</div>'}</article>`).join('')||'<div class="empty">No model status is available.</div>';
  const definitions=learning.definitions||{};$('adminLearningDefinitions').innerHTML=Object.entries(definitions).map(([key,value])=>`<p><b>${esc(key.replaceAll('_',' '))}:</b> ${esc(value)}</p>`).join('');
}
function renderCameraAiPolicy(policy={}){
  const form=$('cameraAiPolicyForm');if(!form)return;
  const metrics=policy.metrics||{},requirements=policy.requirements||{},gates=policy.gates||{};
  $('cameraAiMode').textContent=String(policy.mode||'shadow').replaceAll('_',' ');
  $('cameraAiNote').textContent=policy.primary_active?`Local camera AI is primary. ${policy.fallback} remains automatic fallback.`:policy.primary_requested?`Primary mode is requested and queued. Shadow scoring continues until every evidence gate passes.`:`Local camera AI is in ${policy.mode||'shadow'} mode; ${policy.fallback||'the established service'} remains authoritative.`;
  $('cameraAiMetrics').innerHTML=`<article class="metric"><span>Paired decisions</span><strong>${Number(metrics.comparisons||0)}</strong><small>Need ${Number(requirements.comparisons||25)}</small></article><article class="metric"><span>Agreement</span><strong>${metrics.agreement_pct==null?'—':`${Number(metrics.agreement_pct).toFixed(1)}%`}</strong><small>Target ≥ ${Number(requirements.agreement_pct||90)}%</small></article><article class="metric"><span>Weekly checks</span><strong>${Number(policy.weekly_checks||0)}</strong><small>Need ${Number(requirements.weekly_checks||4)}</small></article><article class="metric"><span>Local failures</span><strong>${metrics.local_failure_pct==null?'—':`${Number(metrics.local_failure_pct).toFixed(1)}%`}</strong><small>Limit ≤ ${Number(requirements.max_local_failure_pct||5)}%</small></article>`;
  form.elements.shadow_enabled.checked=Boolean(policy.shadow_enabled);form.elements.weekly_check_enabled.checked=Boolean(policy.weekly_check_enabled);form.elements.primary_requested.checked=Boolean(policy.primary_requested);
  const labels={shadow_period_complete:'30-day shadow period',weekly_checks_complete:'Four weekly validations',comparison_count_complete:'Paired decision volume',agreement_complete:'Agreement threshold',failure_rate_complete:'Local failure threshold'};
  $('cameraAiGates').innerHTML=Object.entries(labels).map(([key,label])=>`<div class="list-row"><span class="list-icon">${gates[key]?'✓':'·'}</span><div><b>${esc(label)}</b><small>${gates[key]?'Satisfied':'Still collecting evidence'}</small></div></div>`).join('');
  form.onsubmit=async event=>{event.preventDefault();const button=event.submitter;button.disabled=true;try{const result=await api('api/v1/admin/camera-ai-policy',{method:'PUT',body:JSON.stringify({shadow_enabled:form.elements.shadow_enabled.checked,weekly_check_enabled:form.elements.weekly_check_enabled.checked,primary_requested:form.elements.primary_requested.checked})});toast(result.primary_active?'Local camera AI is primary':'Camera AI policy saved; evidence guard remains active');await loadAdminAi()}catch(error){toast(error.message)}finally{button.disabled=false}};
  $('cameraAiCheckNow').onclick=async event=>{const button=event.currentTarget;button.disabled=true;try{const result=await api('api/v1/admin/camera-ai-policy/check',{method:'POST',body:'{}'});toast(result.checked?'Weekly camera AI scorecard saved':result.reason||'Check deferred');await loadAdminAi()}catch(error){toast(error.message)}finally{button.disabled=false}};
}
async function loadAdminAi(){if(!state.session?.permissions?.admin)return null;const data=await optionalApi(`api/v1/admin/ai?_=${Date.now()}`,null);if(data){state.adminAi=data;renderAdminAi(data)}return data}
function queueAiCreditRecheck(button,delay=120000){
  if(aiCreditRecheckTimer)return;
  aiCreditRecheckTimer=setTimeout(()=>{aiCreditRecheckTimer=null;if(document.hidden||!document.querySelector('#view-admin-ai.active')){queueAiCreditRecheck(button,15000);return}button.click()},delay);
}
