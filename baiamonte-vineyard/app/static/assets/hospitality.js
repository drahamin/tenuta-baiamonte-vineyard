(function(){
  ...
  openAdminPerson=function(key){originalOpenAdminPerson(key);const form=$('personAccessForm'),person=(state.adminControl?.people_directory||[]).find(item=>item.key===key);if(!form)return;const select=form.elements.access_level;if(select&&!select.querySelector('[value="hospitality"]'))select.querySelector('[value="worker"]')?.insertAdjacentHTML('beforebegin','<option value="hospitality">Hospitality</option>');if(select&&!select.querySelector('[value="contractor"]'))select.querySelector('[value="worker"]')?.insertAdjacentHTML('afterend','<option value="contractor">Contractor dashboard</option>');if(select&&person)select.value=person.access_level||'viewer';const username=form.elements.username;if(username&&person?.ha_user_id&&person?.username){username.readOnly=true;username.title='Managed by Home Assistant';}const header=form.querySelector('header small');if(header)header.textContent='Home Assistant is authoritative for name, username and identity. This profile assigns application access, estate role and approval responsibilities.'};
