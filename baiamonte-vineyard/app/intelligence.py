...
from .harvest_learning import HARVEST_ANCHORS, build_gdd_curves, estimate_lab_pick_date, fit_harvest_model, prepare_training_rows, summarize_lab_series
...
def refresh_harvest_projections() -> dict[str, Any]:
    ...
    for item in evidence:
        ...
        lab_pick_date = _harvest_date(lab_timing.get("predicted_pick_date")) if lab_timing.get("usable") else None
        lab_adjustment = max(-7, min(7, (lab_pick_date - predicted).days)) if lab_pick_date else 0
        ...
        final_date = max(today, seasonal_start, min(predicted + timedelta(days=weather_adjustment + lab_adjustment + ai_adjustment), seasonal_end))
