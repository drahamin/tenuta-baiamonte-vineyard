...
from .harvest_learning import HARVEST_ANCHORS, build_gdd_curves, estimate_lab_pick_date, fit_harvest_model, fuse_harvest_dates, prepare_training_rows, summarize_lab_series
...
def refresh_harvest_projections() -> dict[str, Any]:
    ...
    for item in evidence:
        ...
        item["lab_timing"] = estimate_lab_pick_date(
            item["lab_statistics"],
            [*(item.get("historical_grape_labs") or []), *(item.get("historical_estate_grape_labs") or [])],
            learning_records,
            item.get("variety") or "",
        )
    ...
    for item in evidence:
        ...
        lab_timing = item.get("lab_timing") or {}
        ...
        ai_suggested_adjustment = int(ai.get("adjustment_days") or 0) if has_current_fruit_evidence else 0
        # When a current approved grape report is available, the deterministic
        # fusion above already embodies that evidence. Applying the narrative
        # AI adjustment again double-counted the same report and caused the
        # overly aggressive dates this pipeline replaces.
        ai_adjustment = 0 if lab_pick_date else ai_suggested_adjustment
        seasonal_start, seasonal_end = date(today.year, 8, 15), date(today.year, 10, 31)
        final_date = max(today, seasonal_start, min(fused_prediction + timedelta(days=weather_adjustment + ai_adjustment), seasonal_end))
...
def analyze_intake(record_id: str, *, allow_reanalysis: bool = False) -> dict[str, Any]:
    ...
    prompt = (
        ...
        "Normalize sample_type to exactly one API value: grape, must, wine, soil, water, or other. In particular, Italian UVA/uve means grape, mosto means must, and vino means wine; preserve the source wording in source_sample_label or notes rather than sample_type. "
