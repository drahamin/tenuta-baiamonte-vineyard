...
def tank_label_rows(year: int, active: bool = True) -> list[dict[str, Any]]:
    rows = fetch_all(
        ...
        "w.stage,w.volume_l,w.variety_summary,s.vintage_year,lp.wine_type,lp.origin_country,"
    ...
    for row in rows:
        ...
        row["processing_phase"] = row.get("processing_phase") or row.get("stage")
...
def save_legal_profile(container_id: str, payload: dict[str, Any], actor: str) -> dict[str, Any]:
    ...
    values = {
        ...
        "processing_phase": str(payload.get("processing_phase") or tank.get("stage") or "").strip() or None,
    ...
    with transaction() as (_, cursor):
        ...
        cursor.execute(
            ...
            "(id,estate_id,wine_lot_id,wine_type,vintage_year,origin_country,denomination_class,denomination,content_description,processing_phase,racking_history,legal_notes,confirmed_by,confirmed_at) "
            "VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,NOW(6)) "
            "ON DUPLICATE KEY UPDATE wine_type=VALUES(wine_type),vintage_year=VALUES(vintage_year),origin_country=VALUES(origin_country),"
            ...
            (new_id(), estate_id(), wine_lot_id, values["wine_type"], vintage, values["origin_country"], values["denomination_class"], values["denomination"], values["content_description"], values["processing_phase"], values["racking_history"], values["legal_notes"], actor),
        )
...
def tank_label_payload(token: str) -> dict[str, Any] | None:
    row = fetch_one(
        ...
        "cp.manual_brix brix,cp.manual_ph ph,cp.manual_reading_at reading_at,lp.wine_type,COALESCE(lp.origin_country,'Italia') origin_country,"
