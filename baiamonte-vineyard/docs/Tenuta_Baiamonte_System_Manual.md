...
The exploratory analyte chart overlays each vintage's recorded first-pick date as a labeled vertical harvest line; a dashed line identifies a current forecast when an actual pick is not yet recorded. Select a measured point in the exploratory, trajectory or like-for-like endpoint chart to open its retained original laboratory report. If an older source file is not retained, the same action opens the complete saved sample and result detail instead. Process-wide annual averages remain summaries and do not pretend to link to a single report.
